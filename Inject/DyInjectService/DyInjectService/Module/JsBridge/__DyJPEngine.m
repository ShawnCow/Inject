...
@implementation __DyJPBoxing

#define __DyJPBoxing_GEN(_name, _prop, _type) \
+ (instancetype)_name:(_type)obj  \
{   \
    __DyJPBoxing *boxing = [[__DyJPBoxing alloc] init]; \
    boxing._prop = obj;   \
    return boxing;  \
}

__DyJPBoxing_GEN(boxObj, obj, id)
__DyJPBoxing_GEN(boxPointer, pointer, void *)
__DyJPBoxing_GEN(boxClass, cls, Class)
__DyJPBoxing_GEN(boxWeakObj, weakObj, id)
__DyJPBoxing_GEN(boxAssignObj, assignObj, id)
...
void (^_dyexceptionBlock)(NSString *log) = ^void(NSString *log) {
    NSCAssert(NO, log);
};

@implementation __DyJPEngine
...
+ (void)startEngine
{
    ...
    context[@"__weak"] = ^id(JSValue *jsval) {
        ...
        return [[JSContext currentContext][@"_formatOCToJS"] callWithArguments:@[formatOCToJS([__DyJPBoxing boxWeakObj:obj])]];
    };
    ...
    context[@"include"] = ^(NSString *filePath) {
        ...
        if (absolutePath && ![_runnedScript containsObject:absolutePath]) {
            [__DyJPEngine _evaluateScriptWithPath:absolutePath];
    ...
    context[@"releaseTmpObj"] = ^void(JSValue *jsVal) {
        if ([[jsVal toObject] isKindOfClass:[NSDictionary class]]) {
            void *pointer =  [(__DyJPBoxing *)([jsVal toObject][@"__obj"]) unboxPointer];
    ...
    context[@"_OC_catch"] = ^(JSValue *msg, JSValue *stack) {
        _dyexceptionBlock([NSString stringWithFormat:@"js exception, \nmsg: %@, \nstack: \n %@", [msg toObject], [stack toObject]]);
    };
    
    context.exceptionHandler = ^(JSContext *con, JSValue *exception) {
        NSLog(@"%@", exception);
        _dyexceptionBlock([NSString stringWithFormat:@"js exception: %@", exception]);
    };
    ...
    if (!path) _dyexceptionBlock(@"can't find JSPatch.js");
...
+ (JSValue *)_evaluateScript:(NSString *)script withSourceURL:(NSURL *)resourceURL
{
    if (!script || ![JSContext class]) {
        _dyexceptionBlock(@"script is nil");
        return nil;
    }
    [self startEngine];
    ...
    @catch (NSException *exception) {
        _dyexceptionBlock([NSString stringWithFormat:@"%@", exception]);
    }
    return nil;
}
...
+ (void)addExtensions:(NSArray *)extensions
{
    ...
    if (!_context) _dyexceptionBlock(@"please call [__DyJPEngine startEngine]");
...
{
    _dyexceptionBlock = [exceptionBlock copy];
}
                ...
                for (NSInteger i = 0; i < argStrArr.count; i++) {
                    ...
                    if (!argEncode) {
                        ...
                        if (NSClassFromString(argClassName) != NULL) {
                            ...
                            _dyexceptionBlock([NSString stringWithFormat:@"unreconized type %@", argStr]);
                            return;
                        }
                    }
...
static NSDictionary *defineClass(NSString *classDeclaration, JSValue *instanceMethods, JSValue *classMethods)
{
    ...
    if (!cls) {
        ...
        if (!superCls) {
            _dyexceptionBlock([NSString stringWithFormat:@"can't find the super class %@", superClassName]);
...
static void JPForwardInvocation(__unsafe_unretained id assignSlf, SEL selector, NSInvocation *invocation)
{
    ...
    if ([slf class] == slf) {
        ...
        [argList addObject:[__DyJPBoxing boxAssignObj:slf]];
        deallocFlag = YES;
    } else {
        [argList addObject:[__DyJPBoxing boxWeakObj:slf]];
    }
    ...
    for (NSUInteger i = 2; i < numberOfArguments; i++) {
        ...
        switch(argumentType[0] == 'r' ? argumentType[1] : argumentType[0]) {
            ...
            case '*': {
                void *arg;
                [invocation getArgument:&arg atIndex:i];
                [argList addObject:[__DyJPBoxing boxPointer:arg]];
                break;
            }
            case '#': {
                Class arg;
                [invocation getArgument:&arg atIndex:i];
                [argList addObject:[__DyJPBoxing boxClass:arg]];
                break;
            }
            default: {
    ...
    switch (returnType[0] == 'r' ? returnType[1] : returnType[0]) {
        ...
        #define JP_FWD_RET_CODE_POINTER    \
            void *ret; \
            id obj = formatJSToOC(jsval); \
            if ([obj isKindOfClass:[__DyJPBoxing class]]) { \
                ret = [((__DyJPBoxing *)obj) unboxPointer]; \
            }
        ...
        #define JP_FWD_RET_CODE_CLASS    \
            ...
            id obj = formatJSToOC(jsval); \
            if ([obj isKindOfClass:[__DyJPBoxing class]]) { \
                ret = [((__DyJPBoxing *)obj) unboxClass]; \
            }
...
#pragma clang diagnostic pop
    ...
    if ([slf respondsToSelector:origForwardSelector]) {
        ...
        if (!methodSignature) {
            _dyexceptionBlock([NSString stringWithFormat:@"unrecognized selector -ORIGforwardInvocation: for instance %@", slf]);
            return;
        }
...
static id callSelector(NSString *className, NSString *selectorName, JSValue *arguments, JSValue *instance, BOOL isSuper)
{
    ...
    if (instance) {
        ...
        if (!instance || instance == _nilObj || [instance isKindOfClass:[__DyJPBoxing class]]) return @{@"__isNil": @(YES)};
    }
    ...
    if (instance) {
        ...
        [_JSMethodSignatureLock unlock];
        if (!methodSignature) {
            _dyexceptionBlock([NSString stringWithFormat:@"unrecognized selector %@ for instance %@", selectorName, instance]);
            return nil;
        }
        ...
        methodSignature = fixSignature(methodSignature);
        if (!methodSignature) {
            _dyexceptionBlock([NSString stringWithFormat:@"unrecognized selector %@ for class %@", selectorName, className]);
            return nil;
        }
        invocation= [NSInvocation invocationWithMethodSignature:methodSignature];
    ...
    for (NSUInteger i = 2; i < numberOfArguments; i++) {
        ...
        switch (argumentType[0] == 'r' ? argumentType[1] : argumentType[0]) {
            ...
            case '*':
            case '^': {
                if ([valObj isKindOfClass:[__DyJPBoxing class]]) {
                    void *value = [((__DyJPBoxing *)valObj) unboxPointer];
            ...
            case '#': {
                if ([valObj isKindOfClass:[__DyJPBoxing class]]) {
                    Class value = [((__DyJPBoxing *)valObj) unboxClass];
                    [invocation setArgument:&value atIndex:i];
                    break;
                }
            }
    ...
    if ([_markArray count] > 0) {
        for (__DyJPBoxing *box in _markArray) {
        ...
        if (strncmp(returnType, "@", 1) == 0) {
            ...
            switch (returnType[0] == 'r' ? returnType[1] : returnType[0]) {
                ...
                case '^': {
                    void *result;
                    [invocation getReturnValue:&result];
                    returnValue = formatOCToJS([__DyJPBoxing boxPointer:result]);
                ...
                case '#': {
                    Class result;
                    [invocation getReturnValue:&result];
                    returnValue = formatOCToJS([__DyJPBoxing boxClass:result]);
                    break;
                }
            }
            return returnValue;
...
id (*_dynew_msgSend1)(id, SEL, id,...) = (id (*)(id, SEL, id,...)) objc_msgSend;
id (*_dynew_msgSend2)(id, SEL, id, id,...) = (id (*)(id, SEL, id, id,...)) objc_msgSend;
id (*_dynew_msgSend3)(id, SEL, id, id, id,...) = (id (*)(id, SEL, id, id, id,...)) objc_msgSend;
id (*_dynew_msgSend4)(id, SEL, id, id, id, id,...) = (id (*)(id, SEL, id, id, id, id,...)) objc_msgSend;
id (*_dynew_msgSend5)(id, SEL, id, id, id, id, id,...) = (id (*)(id, SEL, id, id, id, id, id,...)) objc_msgSend;
id (*_dynew_msgSend6)(id, SEL, id, id, id, id, id, id,...) = (id (*)(id, SEL, id, id, id, id, id, id,...)) objc_msgSend;
id (*_dynew_msgSend7)(id, SEL, id, id, id, id, id, id, id,...) = (id (*)(id, SEL, id, id, id, id, id, id,id,...)) objc_msgSend;
id (*_dynew_msgSend8)(id, SEL, id, id, id, id, id, id, id, id,...) = (id (*)(id, SEL, id, id, id, id, id, id, id, id,...)) objc_msgSend;
id (*_dynew_msgSend9)(id, SEL, id, id, id, id, id, id, id, id, id,...) = (id (*)(id, SEL, id, id, id, id, id, id, id, id, id, ...)) objc_msgSend;
id (*_dynew_msgSend10)(id, SEL, id, id, id, id, id, id, id, id, id, id,...) = (id (*)(id, SEL, id, id, id, id, id, id, id, id, id, id,...)) objc_msgSend;
...
static id invokeVariableParameterMethod(NSMutableArray *origArgumentsList, NSMethodSignature *methodSignature, id sender, SEL selector) {
    ...
    #define JP_CALL_MSGSEND_ARG1(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0));
    #define JP_CALL_MSGSEND_ARG2(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1));
    #define JP_CALL_MSGSEND_ARG3(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1), JP_G_ARG(2));
    #define JP_CALL_MSGSEND_ARG4(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1), JP_G_ARG(2), JP_G_ARG(3));
    #define JP_CALL_MSGSEND_ARG5(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1), JP_G_ARG(2), JP_G_ARG(3), JP_G_ARG(4));
    #define JP_CALL_MSGSEND_ARG6(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1), JP_G_ARG(2), JP_G_ARG(3), JP_G_ARG(4), JP_G_ARG(5));
    #define JP_CALL_MSGSEND_ARG7(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1), JP_G_ARG(2), JP_G_ARG(3), JP_G_ARG(4), JP_G_ARG(5), JP_G_ARG(6));
    #define JP_CALL_MSGSEND_ARG8(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1), JP_G_ARG(2), JP_G_ARG(3), JP_G_ARG(4), JP_G_ARG(5), JP_G_ARG(6), JP_G_ARG(7));
    #define JP_CALL_MSGSEND_ARG9(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1), JP_G_ARG(2), JP_G_ARG(3), JP_G_ARG(4), JP_G_ARG(5), JP_G_ARG(6), JP_G_ARG(7), JP_G_ARG(8));
    #define JP_CALL_MSGSEND_ARG10(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1), JP_G_ARG(2), JP_G_ARG(3), JP_G_ARG(4), JP_G_ARG(5), JP_G_ARG(6), JP_G_ARG(7), JP_G_ARG(8), JP_G_ARG(9));
    #define JP_CALL_MSGSEND_ARG11(_num) results = _dynew_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1), JP_G_ARG(2), JP_G_ARG(3), JP_G_ARG(4), JP_G_ARG(5), JP_G_ARG(6), JP_G_ARG(7), JP_G_ARG(8), JP_G_ARG(9), JP_G_ARG(10));
    ...
    for (int i = 0; i < itemKeys.count; i ++) {
        switch(structTypes[i]) {
            ...
            case '^': {
                ...
                void *val = [(__DyJPBoxing *)dict[itemKeys[i]] unboxPointer];
                memcpy(structData + position, &val, size);
                break;
            }
    ...
    for (int i = 0; i < itemKeys.count; i ++) {
        switch(structTypes[i]) {
            ...
            case '^': {
                ...
                memcpy(val, structData + position, size);
                [dict setObject:[__DyJPBoxing boxPointer:val] forKey:itemKeys[i]];
                position += size;
                break;
            }
...
static id formatOCToJS(id obj)
{
    if ([obj isKindOfClass:[NSString class]] || [obj isKindOfClass:[NSDictionary class]] || [obj isKindOfClass:[NSArray class]] || [obj isKindOfClass:[NSDate class]]) {
        return _autoConvert ? obj: _wrapObj([__DyJPBoxing boxObj:obj]);
    }
...
static id formatJSToOC(JSValue *jsval)
{
    ...
    if ([obj isKindOfClass:[__DyJPBoxing class]]) return [obj unbox];
    ...
    if ([obj isKindOfClass:[NSDictionary class]]) {
        if (obj[@"__obj"]) {
            ...
            if ([ocObj isKindOfClass:[__DyJPBoxing class]]) return [ocObj unbox];
            return ocObj;
        }
        if (obj[@"__isBlock"]) {
...
static NSDictionary *_wrapObj(id obj)
{
    ...
    return @{@"__obj": obj, @"__clsName": NSStringFromClass([obj isKindOfClass:[__DyJPBoxing class]] ? [[((__DyJPBoxing *)obj) unbox] class]: [obj class])};
}
...
@implementation __DyJPExtension
...
+ (void *)formatPointerJSToOC:(JSValue *)val
{
    id obj = [val toObject];
    if ([obj isKindOfClass:[NSDictionary class]]) {
        if (obj[@"__obj"] && [obj[@"__obj"] isKindOfClass:[__DyJPBoxing class]]) {
            return [(__DyJPBoxing *)(obj[@"__obj"]) unboxPointer];
        } else {
            return NULL;
        }
    } else if (![val toBool]) {
        ...
        return [((__DyJPBoxing *)[val toObject]) unboxPointer];
    }
}

+ (id)formatRetainedCFTypeOCToJS:(CFTypeRef)CF_CONSUMED type
{
    return formatOCToJS([__DyJPBoxing boxPointer:(void *)type]);
}

+ (id)formatPointerOCToJS:(void *)pointer
{
    return formatOCToJS([__DyJPBoxing boxPointer:pointer]);
}
