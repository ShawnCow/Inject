...
@class XXAsyncSendPacket;
@class XXAsyncReceivePacket;

extern NSString *const XXAsyncUdpSocketException;
extern NSString *const XXAsyncUdpSocketErrorDomain;

enum XXAsyncUdpSocketError
{
	XXAsyncUdpSocketCFSocketError = kCFSocketError,	// From CFSocketError enum
	XXAsyncUdpSocketNoError = 0,						// Never used
	XXAsyncUdpSocketBadParameter,                     // Used if given a bad parameter (such as an improper address)
	XXAsyncUdpSocketIPv4Unavailable,                  // Used if you bind/connect using IPv6 only
	XXAsyncUdpSocketIPv6Unavailable,                  // Used if you bind/connect using IPv4 only (or iPhone)
	XXAsyncUdpSocketSendTimeoutError,
	XXAsyncUdpSocketReceiveTimeoutError
};
typedef enum XXAsyncUdpSocketError XXAsyncUdpSocketError;

@interface XXAsyncUdpSocket : NSObject
{
	...
	XXAsyncSendPacket *theCurrentSend;
	NSTimer *theSendTimer;
	
	NSMutableArray *theReceiveQueue;
	XXAsyncReceivePacket *theCurrentReceive;
...
/**
 * Creates new instances of XXAsyncUdpSocket.
**/
- (id)init;
...
/**
 * Creates new instances of XXAsyncUdpSocket that support only IPv4 or IPv6.
...
/**
 * When you create an XXAsyncUdpSocket, it is added to the runloop of the current thread.
...
@interface NSObject (XXAsyncUdpSocketDelegate)

/**
 * Called when the datagram with the given tag has been sent.
**/
- (void)onUdpSocket:(XXAsyncUdpSocket *)sock didSendDataWithTag:(long)tag;

/**
 * Called if an error occurs while trying to send a datagram.
 * This could be due to a timeout, or something more serious such as the data being too large to fit in a sigle packet.
**/
- (void)onUdpSocket:(XXAsyncUdpSocket *)sock didNotSendDataWithTag:(long)tag dueToError:(NSError *)error;
 ...
 * and XXAsyncUdpSocket will continue as if the packet never arrived.
...
**/
- (BOOL)onUdpSocket:(XXAsyncUdpSocket *)sock didReceiveData:(NSData *)data withTag:(long)tag fromHost:(NSString *)host port:(UInt16)port;
...
**/
- (void)onUdpSocket:(XXAsyncUdpSocket *)sock didNotReceiveDataWithTag:(long)tag dueToError:(NSError *)error;

/**
 * Called when the socket is closed.
 * A socket is only closed if you explicitly call one of the close methods.
**/
- (void)onUdpSocketDidClose:(XXAsyncUdpSocket *)sock;
