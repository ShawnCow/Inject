...
NSString *const XXAsyncUdpSocketException = @"XXAsyncUdpSocketException";
NSString *const XXAsyncUdpSocketErrorDomain = @"XXAsyncUdpSocketErrorDomain";

#if MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_5
// Mutex lock used by all instances of XXAsyncUdpSocket, to protect getaddrinfo.
// Prior to Mac OS X 10.5 this method was not thread-safe.
static NSString *getaddrinfoLock = @"lock";
#endif

enum XXAsyncUdpSocketFlags
{
...
@interface XXAsyncUdpSocket (Private)
...
/**
 * The XXAsyncSendPacket encompasses the instructions for a single send/write.
**/
@interface XXAsyncSendPacket : NSObject
{
@public
	NSData *buffer;
	NSData *address;
...
@implementation XXAsyncSendPacket
...
/**
 * The XXAsyncReceivePacket encompasses the instructions for a single receive/read.
**/
@interface XXAsyncReceivePacket : NSObject
{
@public
	NSTimeInterval timeout;
	long tag;
...
@implementation XXAsyncReceivePacket
...
@implementation XXAsyncUdpSocket
...
- (BOOL)bindToAddress:(NSString *)host port:(UInt16)port error:(NSError **)errPtr
{
	...
	{
		[NSException raise:XXAsyncUdpSocketException
...
- (BOOL)connectToAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	if(errPtr)
	{
		...
		*errPtr = [NSError errorWithDomain:XXAsyncUdpSocketErrorDomain
									  code:XXAsyncUdpSocketBadParameter
								  userInfo:info];
	}
	return NO;
}
...
- (NSError *)getSocketError
{
	...
	return [NSError errorWithDomain:XXAsyncUdpSocketErrorDomain code:XXAsyncUdpSocketCFSocketError userInfo:info];
}
...
- (NSError *)getIPv4UnavailableError
{
	...
	return [NSError errorWithDomain:XXAsyncUdpSocketErrorDomain code:XXAsyncUdpSocketIPv4Unavailable userInfo:info];
}
...
- (NSError *)getIPv6UnavailableError
{
	...
	return [NSError errorWithDomain:XXAsyncUdpSocketErrorDomain code:XXAsyncUdpSocketIPv6Unavailable userInfo:info];
}
...
- (NSError *)getSendTimeoutError
{
	...
	return [NSError errorWithDomain:XXAsyncUdpSocketErrorDomain code:XXAsyncUdpSocketSendTimeoutError userInfo:info];
}
- (NSError *)getReceiveTimeoutError
{
	...
	return [NSError errorWithDomain:XXAsyncUdpSocketErrorDomain code:XXAsyncUdpSocketReceiveTimeoutError userInfo:info];
}
...
- (BOOL)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	XXAsyncSendPacket *packet = [[XXAsyncSendPacket alloc] initWithData:data address:nil timeout:timeout tag:tag];
...
- (BOOL)sendData:(NSData *)data toHost:(NSString *)host port:(UInt16)port withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	XXAsyncSendPacket *packet = nil;
	
	if(address4 && theSocket4)
		packet = [[XXAsyncSendPacket alloc] initWithData:data address:address4 timeout:timeout tag:tag];
	else if(address6 && theSocket6)
		packet = [[XXAsyncSendPacket alloc] initWithData:data address:address6 timeout:timeout tag:tag];
	else
		return NO;
...
- (BOOL)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	XXAsyncSendPacket *packet = [[XXAsyncSendPacket alloc] initWithData:data address:remoteAddr timeout:timeout tag:tag];
...
- (CFSocketRef)socketForPacket:(XXAsyncSendPacket *)packet
{
	if(!theSocket4)
		return theSocket6;
	if(!theSocket6)
...
- (void)receiveWithTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	XXAsyncReceivePacket *packet = [[XXAsyncReceivePacket alloc] initWithTimeout:timeout tag:tag];
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketWriteCallBack:
			...
			break;
		default:
			NSLog (@"XXAsyncUdpSocket %p received unexpected CFSocketCallBackType %lu.", self, type);
			break;
	}
}
...
static void MyCFSocketCallback(CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	XXAsyncUdpSocket *theSocket = [[(XXAsyncUdpSocket *)pInfo retain] autorelease];
