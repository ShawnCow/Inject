...
@class XXAsyncSocket;
@class XXAsyncReadPacket;
@class XXAsyncWritePacket;

extern NSString *const XXAsyncSocketException;
extern NSString *const XXAsyncSocketErrorDomain;

enum XXAsyncSocketError
{
	XXAsyncSocketCFSocketError = kCFSocketError,	// From CFSocketError enum.
	XXAsyncSocketNoError = 0,						// Never used.
	XXAsyncSocketCanceledError,					// onSocketWillConnect: returned NO.
	XXAsyncSocketConnectTimeoutError,
	XXAsyncSocketReadMaxedOutError,               // Reached set maxLength without completing
	XXAsyncSocketReadTimeoutError,
	XXAsyncSocketWriteTimeoutError
};
typedef enum XXAsyncSocketError XXAsyncSocketError;

@interface NSObject (XXAsyncSocketDelegate)
...
**/
- (void)onSocket:(XXAsyncSocket *)sock willDisconnectWithError:(NSError *)err;
...
**/
- (NSRunLoop *)onSocket:(XXAsyncSocket *)sock wantsRunLoopForNewSocket:(XXAsyncSocket *)newSocket;

/**
 * Called when a socket is about to connect. This method should return YES to continue, or NO to abort.
 * If aborted, will result in XXAsyncSocketCanceledError.
 * 
...
**/
- (BOOL)onSocketWillConnect:(XXAsyncSocket *)sock;
...
**/
- (void)onSocket:(XXAsyncSocket *)sock didReadPartialDataOfLength:(CFIndex)partialLength tag:(long)tag;
 ...
 * It may be used to for things such as updating progress bars.
**/
- (void)onSocket:(XXAsyncSocket *)sock didWritePartialDataOfLength:(CFIndex)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)onSocket:(XXAsyncSocket *)sock
...
**/
- (void)onSocketDidSecure:(XXAsyncSocket *)sock;
...
@interface XXAsyncSocket : NSObject
{
	...
	XXAsyncReadPacket *theCurrentRead;
	NSTimer *theReadTimer;
	NSMutableData *partialReadBuffer;
	
	NSMutableArray *theWriteQueue;
	XXAsyncWritePacket *theCurrentWrite;
...
// Once one of the accept or connect methods are called, the XXAsyncSocket instance is locked in
...
// When an incoming connection is accepted, XXAsyncSocket invokes several delegate methods.
 ...
 * When a connection comes in, the XXAsyncSocket instance will call the various delegate methods (see above).
 ...
 * 
 * Please note the recommended way of releasing an XXAsyncSocket instance (e.g. in a dealloc method)
 * [XXAsyncSocket setDelegate:nil];
 * [XXAsyncSocket disconnect];
 * [XXAsyncSocket release];
**/
- (void)disconnect;
 ...
 * The X509Certificate class is part of the CocoaXXAsyncSocket open source project.
**/
- (void)startTLS:(NSDictionary *)tlsSettings;

/**
 * For handling readDataToData requests, data is necessarily read from the socket in small increments.
 * The performance can be much improved by allowing XXAsyncSocket to read larger chunks at a time and
...
/**
 * When you create an XXAsyncSocket, it is added to the runloop of the current thread.
...
/**
 * Returns the current run loop modes the XXAsyncSocket instance is operating in.
 * The default set of run loop modes is NSDefaultRunLoopMode.
**/
