...
//
//  http://code.google.com/p/cocoaXXAsyncSocket/
//
...
NSString *const XXAsyncSocketException = @"XXAsyncSocketException";
NSString *const XXAsyncSocketErrorDomain = @"XXAsyncSocketErrorDomain";

#if MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_5
// Mutex lock used by all instances of XXAsyncSocket, to protect getaddrinfo.
// Prior to Mac OS X 10.5 this method was not thread-safe.
static NSString *getaddrinfoLock = @"lock";
#endif

enum XXAsyncSocketFlags
{
...
@interface XXAsyncSocket (Private)
...
/**
 * The XXAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface XXAsyncReadPacket : NSObject
{
  @public
...
@implementation XXAsyncReadPacket
...
/**
 * The XXAsyncWritePacket encompasses the instructions for any given write.
**/
@interface XXAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	CFIndex bytesDone;
...
@implementation XXAsyncWritePacket
...
/**
 * The XXAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface XXAsyncSpecialPacket : NSObject
{
  @public
...
@implementation XXAsyncSpecialPacket
...
- (float)progressOfReadReturningTag:(long *)tag bytesDone:(CFIndex *)done total:(CFIndex *)total
{
	...
	// and that the read packet isn't an XXAsyncSpecialPacket (upgrade to TLS).
	if (!theCurrentRead || ![theCurrentRead isKindOfClass:[XXAsyncReadPacket class]]) return NAN;
...
- (float)progressOfWriteReturningTag:(long *)tag bytesDone:(CFIndex *)done total:(CFIndex *)total
{
	...
	// and that the write packet isn't an XXAsyncSpecialPacket (upgrade to TLS).
	if (!theCurrentWrite || ![theCurrentWrite isKindOfClass:[XXAsyncWritePacket class]]) return NAN;
...
- (BOOL)acceptOnInterface:(NSString *)interface port:(UInt16)port error:(NSError **)errPtr
{
	if (theDelegate == NULL)
    {
		[NSException raise:XXAsyncSocketException
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
		...
		// Note: We use [self class] to support subclassing XXAsyncSocket.
		XXAsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
		...
		NSLog(@"XXAsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
							   ...
							   (CFStreamClientContext *)(&theContext)))
	{
		...
		NSLog (@"XXAsyncSocket %p couldn't attach read stream to run-loop,", self);
		NSLog (@"Error: %@", err);
								 ...
								 (CFStreamClientContext *)(&theContext)))
	{
		...
		NSLog (@"XXAsyncSocket %p couldn't attach write stream to run-loop,", self);
		NSLog (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
		NSLog (@"XXAsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen(theWriteStream))
	{
		NSLog (@"XXAsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
 ...
 * When the socket is connected and both streams are open, consider the XXAsyncSocket instance to be ready.
**/
- (void)doStreamOpen
{
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
		...
		{
			NSLog (@"XXAsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	if(peeraddr == NULL)
	{
		NSLog(@"XXAsyncSocket couldn't determine IP version of socket");
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		// Check to see if it's a normal read packet (not XXAsyncSpecialPacket) and if it had read anything yet.
		
		if(([theCurrentRead isKindOfClass:[XXAsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
...
- (NSError *)getSocketError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XXAsyncSocketCFSocketError",
														 @"XXAsyncSocket", [NSBundle mainBundle],
														 @"General CFSocket error", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XXAsyncSocketErrorDomain code:XXAsyncSocketCFSocketError userInfo:info];
}
...
/**
 * Returns a standard XXAsyncSocket abort error.
**/
- (NSError *)getAbortError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XXAsyncSocketCanceledError",
														 @"XXAsyncSocket", [NSBundle mainBundle],
														 @"Connection canceled", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XXAsyncSocketErrorDomain code:XXAsyncSocketCanceledError userInfo:info];
}

/**
 * Returns a standard XXAsyncSocket connect timeout error.
**/
- (NSError *)getConnectTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XXAsyncSocketConnectTimeoutError",
														 @"XXAsyncSocket", [NSBundle mainBundle],
														 @"Attempt to connect to host timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XXAsyncSocketErrorDomain code:XXAsyncSocketConnectTimeoutError userInfo:info];
}

/**
 * Returns a standard XXAsyncSocket maxed out error.
**/
- (NSError *)getReadMaxedOutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XXAsyncSocketReadMaxedOutError",
														 @"XXAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XXAsyncSocketErrorDomain code:XXAsyncSocketReadMaxedOutError userInfo:info];
}

/**
 * Returns a standard XXAsyncSocket read timeout error.
**/
- (NSError *)getReadTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XXAsyncSocketReadTimeoutError",
														 @"XXAsyncSocket", [NSBundle mainBundle],
														 @"Read operation timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XXAsyncSocketErrorDomain code:XXAsyncSocketReadTimeoutError userInfo:info];
}

/**
 * Returns a standard XXAsyncSocket write timeout error.
**/
- (NSError *)getWriteTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XXAsyncSocketWriteTimeoutError",
														 @"XXAsyncSocket", [NSBundle mainBundle],
														 @"Write operation timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XXAsyncSocketErrorDomain code:XXAsyncSocketWriteTimeoutError userInfo:info];
}
...
- (NSString *)description
{
	...
	[ms appendString:[NSString stringWithFormat:@"<XXAsyncSocket %p", self]];
...
- (void)readDataToLength:(CFIndex)length withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	XXAsyncReadPacket *packet = [[XXAsyncReadPacket alloc] initWithData:buffer
															timeout:timeout
																tag:tag
												   readAllAvailable:NO
														 terminator:nil
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
			...
			if([theCurrentRead isKindOfClass:[XXAsyncSpecialPacket class]])
			{
...
- (void)doBytesAvailable
{
	...
	if((theCurrentRead != nil) && (theReadStream != NULL))
	{
		// Note: This method is not called if theCurrentRead is an XXAsyncSpecialPacket (startTLS packet)
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	XXAsyncWritePacket *packet = [[XXAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	...
	if((theCurrentWrite == nil) && (theWriteStream != NULL))
	{
		if([theWriteQueue count] > 0)
		{
			...
			if([theCurrentWrite isKindOfClass:[XXAsyncSpecialPacket class]])
			{
				// Attempt to start TLS
...
- (void)doSendBytes
{
	if((theCurrentWrite != nil) && (theWriteStream != NULL))
	{
		// Note: This method is not called if theCurrentWrite is an XXAsyncSpecialPacket (startTLS packet)
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	XXAsyncSpecialPacket *packet = [[XXAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	{
		XXAsyncSpecialPacket *tlsPacket = (XXAsyncSpecialPacket *)theCurrentRead;
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			NSLog (@"XXAsyncSocket %p received unexpected CFSocketCallBackType %lu.", self, type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog (@"XXAsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %lu.", self, type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog (@"XXAsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %lu.", self, type);
	}
}
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	XXAsyncSocket *theSocket = [[(XXAsyncSocket *)pInfo retain] autorelease];
